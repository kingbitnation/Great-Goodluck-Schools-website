...
function StudentMessagesPage({ user }: { user: AuthUser }) {
      ...
      <div className="p-8 max-w-7xl mx-auto">
        ...
        {showCompose && (
          <div className="bg-white p-6 rounded-lg shadow mb-6 space-y-4">
            <div>
              <label className="block text-sm font-medium mb-2">Send to (Email or User ID)</label>
              <input
                type="text"
                value={composeData.receiverId}
                onChange={(e) => setComposeData({ ...composeData, receiverId: e.target.value })}
                placeholder="Teacher or staff email"
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600">
                      {tab === 'inbox' ? message.sender.email : message.receiver.email}
                    </p>
...
export default withAuth(StudentMessagesPage, { roles: ['Student'] })
